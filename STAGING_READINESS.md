...
| Concurrency, provider throughput (measured, live) | **NEEDS ATTENTION — separate from the row above** | Real load smoke test against the live AWS deployment (2026-09-15): 10 sequential text requests, 9/10 succeeded, 1 timed out at 30s; 2 simultaneous text requests, only 1/2 succeeded, the other timed out at 40s. Container logs confirmed the root cause was `llm_provider_failed`/`LLMError` from Groq itself during that window — CPU stayed at 0.15%, memory at 1.17GB/7.6GB, zero container restarts, so this is not an EC2/resource problem, and it is not fixed by the multi-user work above: VoiceBot correctly keeps two patients' *data* apart regardless, but two *simultaneous LLM calls* (whether for the same patient or different ones) can still both hit Groq at once and one can fail. 2 simultaneous voice requests succeeded (2/2). Concurrency beyond that was not tested. **Recommendation before any multi-user official demo**: add retry-with-backoff around the Groq call, or explicitly warn testers that simultaneous use may produce occasional slow/failed replies. |
