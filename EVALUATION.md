...
**Date:** 2026-09-05
**Result of `pytest -q`:** 280 passed (historical — see the 2026-09-16 section immediately
below for the current count and current live-deployment verification)

---

## Current deployment verification — 2026-09-16
...
1. **Deterministic/mock automated tests** — `pytest -q`: **655 passed, 0 failed**. Every
...
3. **Real deployment verification** — the same real provider calls, but made through the
   ...
   (apply/stale-rejection/conflict-rejection/no-op-replay), dynamic multi-patient
   provisioning and isolation under real simultaneous load, patient isolation, and recovery
   across both a container restart and a full instance reboot. This is the strongest
   evidence in this repository and is what `docs/RELEASE_ACCEPTANCE.md` cites. A
   comprehensive live re-sweep on 2026-09-16 (~30 checks) passed cleanly; the one real
   finding was Groq's own single-call latency variance (up to ~35s with zero concurrency),
   addressed by tuning `SMRITI_MAX_RETRIES` to bound the worst case — see `CHANGELOG.md`.
...
Test count history: 280 (2026-09-05, historical) → 625 (2026-09-15, historical) → 655
(current). The increase reflects real feature work across multiple sessions (persistent
sessions, memory-sync versioning, async voice jobs, response-language honesty, deployment
hardening, dynamic multi-patient provisioning), not re-counting the same tests — see
`CHANGELOG.md` for the dated breakdown.
