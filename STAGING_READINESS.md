...
| Concurrency, application-level isolation | **READY** | Multiple patients sharing one deployment simultaneously is now implemented and tested: `tests/integration/test_dynamic_backend_key_provisioning.py` proves simultaneous conversations, session/memory/job/audio/cancellation isolation, and independent disable/re-enable across patients, all via real HTTP calls against a real (in-test) server — not process-global mutable state anywhere in the request path. |
| Concurrency, provider throughput (measured, live) | **PARTIAL — bounded, not eliminated** | Full re-swept against the live AWS deployment (2026-09-16, ~30 checks across auth, safety, multi-user isolation, simultaneous text, session-swap rejection, disable/re-enable, memory-sync revisioning, prompt-injection resistance, real voice round trip, cancellation, language honesty): every check passed once test-harness contention/bugs were ruled out (see `docs/AWS_DEPLOYMENT.md` §15 "Post-launch reliability tuning"). Simultaneous requests for two different patients now correctly return distinct sessions with correct per-patient content while properly isolated. The remaining limitation is unchanged and is Groq's own: single-call latency on the free/shared tier varies from ~300ms up to ~35s even with zero concurrency, confirmed by isolating one non-retried call. Because `SMRITI_LLM_PROVIDER=groq` is configured explicitly in production (no fallback chain), a slow/failing Groq call has no other provider to fall back to — it degrades gracefully to an `llm_unavailable` response rather than hanging, but that response can itself take time to arrive. **Fix applied (2026-09-16)**: `SMRITI_MAX_RETRIES` was unset in production (defaulting to `2`, worst case ~90s of retries at the SDK level); set explicitly to `1`, halving the worst case to ~60s without reducing single-attempt reliability. This bounds the problem; it does not eliminate Groq's own latency variance, which is outside VoiceBot's control on the free tier. |
